...
packages/
  ...
  ode-core/              Adaptive non-stiff IVP integration on Float64Array. Substrate for tools/integrate-ode-ivp. Dormand-Prince 5(4) (HNW Vol I §II.5) with FSAL bookkeeping, Gustafsson 1991 PI step-size controller, 4th-order Hermite continuous extension (HNW §II.6) for dense output. The SciPy `solve_ivp(method='RK45')` algorithm in pure TS. Closed-vocabulary RHS evaluation reuses `@workbench/quadrature::evalNumericExpr`.
