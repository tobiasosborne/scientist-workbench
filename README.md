...
| `solve` | `record{eqs: list<expression>, vars: list<symbol>}` | `record{vars, solutions: list<record{bindings, branches}>, completeness, warnings}` \| `tagged "solve/{high-degree-irreducible,multivariate-non-zero-dim,parametric-non-trivial,foreign-vocabulary,constant-equation,empty-input,empty-vars}"` | Top-level Mathematica `Solve[]`-class dispatcher (v0.1). Classifies the input via `@workbench/solve::classifyInput` and dispatches: linear systems → `bareissSolve` (exact Bareiss elimination over ℚ; unique / under-determined-with-branches / inconsistent verdicts), univariate polynomial → `factorRatQ` + radical solvers (deg ≤ 4 closed-form; deg ≥ 5 refused per Galois 1832), multivariate-non-zero-dim / parametric / transcendental → honest boundary tags pending Gröbner / invert-and-substitute substrate. Output shape per ADR-0017 (solution-set: `bindings` per variable + `branches` for integer-parameter families). Branch-honest by design — no silent principal-branch slices; under-determined linear systems return one `Solution` with non-empty `branches`. Substrate `@workbench/solve` (`classifyInput, dispatchClassified`) on top of cas-core / poly-factor. Default symbolic tier. |
