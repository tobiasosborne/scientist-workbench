...
packages/
  ...
  alg-num/               Algebraic-number substrate. v0.1 ships the `Root[poly, k]` primitive (ADR-0018): a canonical naming scheme for individual algebraic numbers via `(minimal polynomial, sort-order index)`. The constructor `makeRoot(poly, intervalHint, v)` accepts any polynomial in ℚ[v] (reducible, rational coefficients, non-monic) plus a real isolating-interval hint, factors via `@workbench/poly-factor::factorRatQ`, selects the unique irreducible factor with a real root in the hint via the sign criterion + VAS-LMQ from `@workbench/real-roots`, then canonicalises that factor to ℤ[x] (clear denominators by LCM, strip integer content by GCD, sign-fix to positive leading coefficient). The wire encoding is `expression { head: "Root", args: [Polynomial[c_0, …, c_n], k] }` with all coefficients integer-typed. Real roots only in v0.1 (complex-root naming awaits complex-root isolation). Lazy interval refinement (xkz), full equality with non-canonical inputs (6cd), and resultant-based `+/·` arithmetic (rti) extend this surface in sibling beads.
