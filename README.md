...
| `cas-diff` | `record{f: expression, var: symbol}` | `expression` \| `tagged "cas-diff/out-of-scope"` | Symbolic differentiation over the closed elementary + special-function vocabulary (ADR-0023; 38 heads after the 2026-05-19 Gamma-family extension per ADR-0042); DLMF-cited chain-rule rules for Gamma, Digamma, LogGamma, IncompleteGamma upper/lower, Beta, Bessel J/Y/I/K + Hankel + spherical, Erf/Erfc/Erfi, Fresnel, ExpIntegral, Hermite, Polylog. See [`tools/cas-diff/`](tools/cas-diff/README.md). |
...
| `cone-solve` | `record{minimize:{c, Q?}, subjectTo:{Ax_eq_b?, cones}, precision?, max_iter?}` per ADR-0030 §C | `record{status, x, dual, slack, objective?, achieved_precision?, iterations, method, condition_estimate, warnings}` \| `tagged "cone-solve/{non-finite-input,degenerate-shape,malformed-cone,unsupported-cone,quadratic-objective}"` | Universal-primary tool of the convex-cone solver tier (ADR-0030 §B) — the symbol an agent types for any convex cone program without first classifying it LP/QP/SOCP/SDP. Wraps `@workbench/cone-core`'s SCS operator-splitting substrate (O'Donoghue-Chu-Parikh-Boyd 2016 homogeneous self-dual embedding) with Ruiz data equilibration (§5) and Type-II Anderson acceleration (ADR-0036). Translates the §C form (`min cᵀx s.t. Ax=b, x∈𝒦`) into cone-core's O'Donoghue form, solves, recovers the §C primal-dual point; the five-class `status` discriminant is the contract (`optimal` carries x/dual/slack/objective, `infeasible` a Farkas cert, `unbounded` a ray). `numerical: true` (ADR-0015). SCS is a modest-accuracy first-order method (ceiling ≈1e-6); tight-tolerance LP/QP is the specialists' job (`lp-solve`, `qp-solve`). v0.1 cone scope is the LP-complete subset (nonneg/zero/free); SOC/PSD/EXP/POW refuse with `unsupported-cone`. See [`tools/cone-solve/`](tools/cone-solve/README.md). |
| `choi-iso` | `tagged "channel-to-choi" record{channel, dim_in, dim_out}` \| `tagged "choi-to-channel" record{J, dim_in, dim_out}` (all matrices `list<list<float64>>`) | `record{J, shape, warnings}` \| `record{channel, shape, warnings}` | Choi–Jamiołkowski isomorphism between superoperator matrices (`d_out² × d_in²`, column-stacking vec) and Choi matrices (`(d_in·d_out)²`). Pure index permutation per Watrous / QuTiP / Wood–Biamonte–Cory Eq. 3.22 convention; bit-identical round-trip is the decisive invariant; `J(T) = SWAP` makes the transpose-map's not-CP-ness visible as eigenvalues `{−1,1,1,1}`. v0.1 of the qinfo tool surface (ADR-0034); pairs with `partial-trace` (CP / TP tests). See [`tools/choi-iso/`](tools/choi-iso/README.md). |
...
| `fidelity` | `record{rho: record{re, im}, sigma: record{re, im}}` (both Hermitian, same n × n) | `record{value: float64, sqrt_value, bures_angle, method, warnings}` \| `tagged "fidelity/{non-hermitian-input,non-finite-input,shape-mismatch,degenerate-shape}"` | Uhlmann fidelity `F(ρ, σ) = (tr √(√ρ · σ · √ρ))²` between two complex Hermitian density operators (Uhlmann 1976; Jozsa 1994). Three `eighComplex` calls — `√ρ` via spectral PSD-clamp, then eigh of `√ρ σ √ρ` (Hermitian PSD), then `(Σ √μ_+)²`. Output surfaces `√F` (Bhattacharyya overlap) and `bures_angle = arccos(√F)` (Riemannian geodesic distance under the Bures metric) as essentially-free byproducts. Fourth and final qinfo v0.2 tool; pairs with `trace-distance` via the Fuchs–van de Graaf inequality `1 − √F ≤ D ≤ √(1 − F)`. PSD violation on either input is a soft warning (negative eigenspaces clamped to 0), not a refusal — agent-honest computation on near-PSD inputs. Pure-state-overlap closed form `F(\|ψ⟩⟨ψ\|, \|φ⟩⟨φ\|) = \|⟨ψ\|φ⟩\|²`; pure-vs-max-mixed `F = 1/d`. See [`tools/fidelity/`](tools/fidelity/README.md). |
...
| `linalg-eigh-complex` | `record{re: list<list<float64>>, im: list<list<float64>>}` (both required, shape-matched) | `record{Q: record{re, im}, eigenvalues: list<float64>, reconstruction_error, orthogonality_error, condition_number, method, warnings}` \| `tagged "linalg-eigh-complex/{non-hermitian-input,non-finite-input,degenerate-shape}"` | Hermitian eigendecomposition `H = Q · diag(λ) · Q†` for a complex `n × n` Hermitian `H` via the real-symplectic embedding (Goedecker 1999; Day & Heroux 2001) composed with the existing cyclic-Jacobi real `eigh`. Eigenvalues are real (Hermitian spectral theorem); eigenvectors complex; agent-honest reconstruction + unitarity diagnostics. ADR-0035 phase 1; the v0.1 algorithm reuses real eigh verbatim. First downstream consumer: `tools/trace-norm` (filed, bead `korg`). See [`tools/linalg-eigh-complex/`](tools/linalg-eigh-complex/README.md). |
...
| `linalg-svd-complex` | `record{re: list<list<float64>>, im: list<list<float64>>, mode?}` (both required, shape-matched) | `record{U: record{re, im}, S: list<float64>, Vh: record{re, im}, mode, reconstruction_error, orthogonality_error_U, orthogonality_error_Vh, condition_number, rank_estimate, method, warnings}` \| `tagged "linalg-svd-complex/{non-finite-input,degenerate-shape}"` | Singular value decomposition `M = U · diag(S) · V†` of a complex `m × n` matrix via complex one-sided Jacobi (Forsythe-Henrici 1960 → Brent-Luk 1985 → Hari-Veselić 1987 → Drmač 1997). Unlike `linalg-eigh-complex`, supports rectangular inputs (the m < n case routes via M† internally) and the v0.1 algorithm is *not* the real-symplectic embedding (ADR-0035 §D8: "the embedding buys nothing for SVD"). `Vh` is the conjugate transpose (NumPy convention, parallel to real `Vt`); `S` is real. Agent-honest reconstruction + per-factor unitarity diagnostics. ADR-0035 phase 2 (bead `jao0`); lifted the v0.1 `trace-norm/non-hermitian-input` refusal. See [`tools/linalg-svd-complex/`](tools/linalg-svd-complex/README.md). |
| `lp-solve` | `record{minimize:{c}, subjectTo:{Ax_eq_b?, cones}, precision?, max_iter?}` per ADR-0030 §C | `record{status, x, dual, slack, objective?, achieved_precision?, iterations, method, condition_estimate, warnings}` \| `tagged "lp-solve/{non-finite-input,degenerate-shape,non-lp-cone,malformed-cone,quadratic-objective,coefficient-explosion}"` | LP specialist of the cone-solver tier (ADR-0030 §B). Three lanes share one wire: `--method=exact` — arbitrary-precision rational two-phase simplex via `@workbench/simplex-q` (Dantzig 1947 + Bland 1977; ADR-0031) — only LP solver in any ecosystem returning `achieved_precision ≈ ε_machine` on small-dense inputs; `--method=ipm` — Mehrotra primal-dual predictor-corrector via `@workbench/solver-ipm` (ADR-0032), scales to NETLIB (m, n ≈ 100–2000); `--method=hsde-lp` — homogeneous self-dual embedding (ART03 / Mosek-aligned; ADR-0033) for infeasibility-detecting and tight-tol regimes. Default `auto` dispatches by problem size (m+n ≤ 50 → exact, else IPM); precision-aware auto-dispatch tracked at bead `9vc9`. See [`tools/lp-solve/`](tools/lp-solve/README.md). |
...
| `partial-trace` | `record{M: list<list<float64>>, dims: list<integer>, trace_out: list<integer>}` | `record{reduced, reduced_dims, shape, warnings}` | Trace out one or more subsystems of an operator on a tensor-product Hilbert space (dims-generalised; qubit 0 = leftmost). Defining property `Tr_1(A⊗B) = tr(B)·A` is the decisive invariant. v0.1 of the qinfo tool surface (ADR-0034); pairs with `tensor-product`. See [`tools/partial-trace/`](tools/partial-trace/README.md). |
| `partial-transpose` | `record{M: list<list<float64>>, dims: list<integer>, transposeOn: list<integer>}` | `record{M_pt, shape, warnings}` | Transpose on selected subsystems of an operator on a tensor-product Hilbert space (dims-generalised; qubit 0 = leftmost). Involution `PT_S(PT_S(M)) = M`; whole-system PT = full transpose. The Peres–Horodecki PPT criterion: PT on one qubit of `\|Φ+⟩⟨Φ+\|` equals `(1/2) SWAP`, min eigenvalue −1/2 — the canonical entanglement witness (necessary-and-sufficient on 2×2 and 2×3). v0.1 of the qinfo tool surface (ADR-0034); pairs with `choi-iso` for the channel-side test. See [`tools/partial-transpose/`](tools/partial-transpose/README.md). |
...
| `purity` | `record{rho: record{re: list<list<float64>>, im: list<list<float64>>}}` (Hermitian) | `record{value: float64, trace: float64, is_pure_within_tolerance: boolean, method, warnings}` \| `tagged "purity/{non-hermitian-input,non-finite-input,degenerate-shape}"` | Purity `γ(ρ) = tr(ρ²)` of a complex Hermitian density operator. For Hermitian ρ this collapses to the one-pass entrywise identity `γ = Σ \|ρ_{ij}\|² = Σ (re² + im²)` — no eigendecomposition needed; O(n²). The named flag `is_pure_within_tolerance = (\|γ − 1\| ≤ 1e-9)` answers "is ρ a rank-1 projector?". The second qinfo v0.2 tool, sibling to `trace-norm`; on density operators `γ ∈ [1/d, 1]` with `γ = 1` ⇔ pure and `γ = 1/d` ⇔ maximally mixed (N&C §8.4; Bengtsson-Życzkowski §2.3). The α=2 Rényi entropy is `−log γ`, so this tool computes its kernel. See [`tools/purity/`](tools/purity/README.md). |
...
| `sdp-solve` | `record{minimize:{c}, subjectTo:{Ax_eq_b?, cones}, precision?, max_iter?}` per ADR-0030 §C with `PSDCone[size, indices]` | `record{status, x, dual, slack, objective?, achieved_precision?, iterations, method, condition_estimate, warnings}` \| `tagged "sdp-solve/{non-finite-input,degenerate-shape,quadratic-objective,non-sdp-cone,malformed-cone,cone-coverage}"` | SDP specialist of the cone-solver tier (ADR-0030 §B). Wraps `@workbench/solver-ipm`'s primal-dual interior-point method with four search-direction lanes — **`--method=auto` (default) routes to HSDE-NT** (homogeneous self-dual embedding per ADR-0033 + iterative refinement per Phase 5 Tiers 1-3, worklogs 106 / 110 / 128 / 129); legacy Nesterov-Todd kept as `--method=nt` for A/B and trace-diff (Todd-Toh-Tütüncü 1998); AHO (Alizadeh-Haeberly-Overton 1997) and HKM (debug-only) round out the panel. HSDE termination has a soft-success branch (μ-test, mirroring legacy NT's `couldDualFeas`) so τ-shrunk iterates at the float64 floor land as wire `optimal` with `achieved_precision` carrying the actual purified ρ-max. Wire uses **strict-Mosek-format with √2 off-diagonal scaling** so `<C, X>_F = svec(C)ᵀ svec(X)` exactly. v0.1 cone vocabulary: PSDCone (+ ZeroCone); NonNegCone-as-diagonal-PSD deferred to v0.2 (bead `67nj`). Bench grade: 5/6 cases, 64/66 invariants on sdp-sdplib (corpus) at default tol — `hinf2 optimality_gap` flips to pass via Tier 1 IR's 3-decade `r_p` improvement; the remaining 2 hinf2 invariants (`primal_feasibility`, `complementary_slackness`) are the float64 representation limit of `r_p / τ` purification, lifted by Phase 6 bigfloat (separate ADR per ADR-0033 Decision 9). See [`tools/sdp-solve/`](tools/sdp-solve/README.md). |
...
| `special-eval` | `record{head: string, args: list<float64> \| record{re, im: list<float64>}}` | `record{value: bigfloat \| bigcomplex, method, achieved_precision, warnings}` \| `tagged "special-eval/{unknown-head,non-finite-input,degenerate-shape,no-known-representation}"` | Per-head arb-prec / float64 evaluator umbrella for the Erf family (Erf, Erfc, Erfcx, Erfi, InverseErf, InverseErfc), the Bessel family (BesselJ/Y/I/K + scaled variants), and — since the 2026-05-19 Gamma-family extension (ADR-0042 §"Decision 7") — the 16-head Gamma family (Gamma, LogGamma, Digamma, Trigamma, Polygamma, Pochhammer, IncompleteGamma upper/lower/P/Q, Beta, LogBeta, BarnesG, Hyperfactorial, GammaRatio, GammaDeltaRatio) — real and complex variants. One tool dispatches across every head and across the {float64, arb-prec} tiers behind a single `--head=<name>` flag; `--precision ≤ 15` (decimal) routes the SunPro-1993 / Faddeeva-Johnson / Blair-1976 float64 lanes; `> 15` routes the Borel-series / Karbach-Weideman arb-prec lanes (`@workbench/bigfloat`). `arbprec: true` — bit-identical cross-platform forever given `--precision=N`. Honest refusal envelope covers unknown heads, non-finite args, degenerate shapes, and the v0.1 substrate gaps (arb-prec inverses, complex inverses). ADR-0040 §"Decision 7"; v0.1 instantiation of the per-head substrate pattern. See [`tools/special-eval/`](tools/special-eval/README.md). |
...
| `sturm-trace` | `record{source: string, entry?: string}` | Sturm channel IR \| `tagged "sturm-trace/{parse-error,invalid-when-body,non-pure-trace,non-channel-return}"` | TS-native frontend tool: take a TypeScript source string that builds a channel via `@workbench/sturm`'s `trace(...)` DSL (ADR-0009), run it in a sandboxed Bun subprocess, and emit the canonical IR Value. The fifth and outermost layer of ADR-0038's coherent-control enforcement — a `when(...) { observe(...) }` body refuses at trace time with `tagged "sturm-trace/invalid-when-body"` carrying `{op_head, control_wires, message}` (the four IR-side layers being schema closure, builder API, decoder arity, and cases-arm well-formedness). Determinism check on by default (traces twice, byte-compares); `--skip-determinism` opts out. Bridges the "source → IR" boundary so agents compose by-pipe (`cat circuit.json \| bun tools/sturm-trace/tool.ts \| bun tools/sturm-execute/tool.ts`) rather than by-direct-IR-construction. See [`tools/sturm-trace/`](tools/sturm-trace/README.md). |
| `tensor-product` | `record{A: list<list<float64>>, B: list<list<float64>>}` | `record{AB, shape, warnings}` | Kronecker product `A ⊗ B` of two real matrices; the standard composable primitive for assembling operators on product spaces. Mixed-product law `(A⊗B)(C⊗D) = (AC)⊗(BD)` is the decisive invariant. v0.1 of the qinfo tool surface (ADR-0034); pairs with `partial-trace`. See [`tools/tensor-product/`](tools/tensor-product/README.md). |
| `trace-distance` | `record{rho: record{re, im}, sigma: record{re, im}}` (both Hermitian, same n × n) | `record{value: float64, method, warnings}` \| `tagged "trace-distance/{non-hermitian-input,non-finite-input,shape-mismatch,degenerate-shape}"` | Trace distance `D(ρ, σ) = ½‖ρ − σ‖₁` between two complex Hermitian density operators. The operationally-meaningful state-distinguishability metric: by Helstrom's theorem (1969) the maximum probability of distinguishing equally-likely ρ vs σ by any measurement is `(1 + D)/2`. Composes the same `eighComplex` substrate as `trace-norm` on the Hermitian difference (`ρ − σ` is Hermitian for Hermitian ρ, σ); spectral formula `D = ½ Σ \|λ_k(ρ − σ)\|` (Bhatia §IV.2 + halving). Third qinfo v0.2 tool. Boundary tag `which` field names rho-vs-sigma on Hermiticity/non-finite failures; shape-mismatch refuses across Hilbert-space dimensions (embedding into a common space is the caller's job). See [`tools/trace-distance/`](tools/trace-distance/README.md). |
| `trace-norm` | `record{M: record{re: list<list<float64>>, im: list<list<float64>>}}` (square; any complex matrix) | `record{value: float64, eigenvalues?, singular_values?, condition_number, method, warnings}` \| `tagged "trace-norm/{non-finite-input,degenerate-shape}"` | Schatten-1 (trace / nuclear) norm `‖M‖₁` of any square complex matrix via two-path dispatch: Hermitian → `linalg-eigh-complex` (`Σ \|λ_k\|`, populates `eigenvalues`), non-Hermitian → `linalg-svd-complex` (`Σ S_k`, populates `singular_values`). The first qinfo v0.2 tool. The non-Hermitian path landed with ADR-0035 phase 2 (bead `jao0`); the v0.1 `trace-norm/non-hermitian-input` refusal class is retired. Workhorse primitive for `trace-distance` (`k2xo`), `fidelity` (`2hxf`), `purity` (`2czd`); the operationally-meaningful state-distinguishability quantity (Helstrom; Fuchs–van de Graaf). See [`tools/trace-norm/`](tools/trace-norm/README.md). |
...
packages/
  ...
  cas-core/              multivariate Q[x_1,…,x_n] / Q(x_1,…,x_n) arithmetic; symbolic differentiator (closed elementary vocabulary + ADR-0023 special-function vocabulary, 38 heads as of 2026-05-19: Gamma family — Gamma, Digamma, Polygamma, LogGamma, Pochhammer, IncompleteGammaUpper/Lower, Beta, BarnesG —, Bessel J/Y/I/K + Hankel H¹/H² + spherical j/y, HypergeometricPFQ, MeijerG, Whittaker, ParabolicCylinderD, Erf/Erfc/Erfi, ExpIntegralEi/E, Fresnel, Legendre, classical orthogonal polynomials, Polylog, LerchPhi); ring-generic Poly<T> / RatFn<T> over Field<T> dictionaries; `Root[poly, k]` algebraic-number primitive (re-exported from @workbench/alg-num)
  ...
  linalg-core/           First numerical-tier package (ADR-0014): dense Float64Array Matrix, LU + partial pivoting, solve with iterative refinement, Hager 1-norm condition estimator. Pure TS, single platform, no FFI. ADR-0035 phase 1 extends with `ComplexMatrix` (`{rows, cols, re, im}` with `im` required, structurally identical to qinfo's `Matrix`-with-im but distinguished at the type level) and `eighComplex` (real-symplectic embedding `H = A+iB → [[A,-B],[B,A]]` reusing the existing real cyclic-Jacobi `eigh` verbatim). Bridge helpers `complexFromReal`, `complexFromQinfo`, `realPartOnly` keep the type discipline at the boundary.
  qinfo/                 Quantum-information substrate (ADR-0034). One `Matrix = {rows, cols, re, im?}` type covers real and complex via an optional imaginary part; complete index-only operation surface: `kron` / `kron2` (Kronecker product, real ⊗ real or complex), `partialTrace` (arbitrary subsystem dims, sequential multi-trace), `partialTracePure` (Schmidt-decomposition O(d·rank) path for state vectors), `partialTranspose` (PPT criterion building block), `vec` / `unvec` / `choi` / `deChoi` (column-stacking convention; channels ↔ Choi matrices). Subsystem 0 is the leftmost tensor factor (locked). Complex from day 1 on the index-only ops (no `linalg-eigh` dependency); eigh-routed metrics (`traceNorm`, `traceDistance`, `fidelity`) blocked on the complex-Hermitian linalg extension (bead `ov4j`). Substrate for `tools/tensor-product` + `tools/partial-trace` + `tools/choi-iso` + `tools/partial-transpose` (v0.1, complete). The first user is the DMTL21 Wasserstein-1 dogfood; the substrate eliminates the 50+ LOC of partial-trace boilerplate that prompted the epic.
  ...
  cone-core/             SCS-style convex-cone solver substrate (ADR-0030 §H), the universal-primary lineage of the cone-solver tier. `scsSolve` runs the O'Donoghue-Chu-Parikh-Boyd 2016 operator-splitting iteration on the homogeneous self-dual embedding: `buildHSDE` validates a `ConeProblem` and structures the embedding (skew-symmetric `Q`), `recoverPrimalDual` is the §3.5 termination evaluator (a primal-dual solution, a Farkas infeasibility certificate, or an unboundedness ray — one discriminated `Recovered`), and the iteration caches the Sherman-Morrison-Woodbury factorisation of `M = [[I, Aᵀ], [−A, I]]` per the §4.1 efficient subspace projection. Two convergence levers wrap the iteration: `scaling.ts` — Ruiz data equilibration (O'Donoghue §5), the iteration runs on the rescaled `Â = D A E` while termination is judged on the original residuals; and `anderson.ts` — Type-II Anderson acceleration (ADR-0036, Zhang-O'Donoghue-Boyd 2018 / Walker-Ni 2011), a generic windowed-least-squares extrapolator for any `Float64Array` fixed-point map, on by default. `numerical: true` (ADR-0015) — a fixed sequence of float64 ops, reproducible given `(problem, opts, platform)`; no implicit-zero gate, every threshold an explicit tolerance derived from the single `precision` knob. v0.1 scope is the LP-complete cone subset — `projectCone` / `dualCone` / `inCone` implement the nonneg / zero / free families (definitional, no second reference needed); the `SOCone` / `PSDCone` / `ExpCone` / `PowCone` variants are in the `Cone` union (the documented surface) but every operation on them throws a loud `ConeError` naming its sub-bead (`0wc7` SOC+PSD, `j282` Exp+Pow — each blocked on staging Parikh-Boyd §6.3). Pure TS on `Float64Array`, depends only on `@workbench/linalg-core` — like linalg-core, a library not a tool, the wire encoding lives in `tools/cone-solve`. Ground truth transcribed to `docs/ground-truth/convex/` from `docs/refs/odonoghue-2016-scs.pdf` and arXiv:1808.03971; ported from the papers, never `scs.c` (ADR-0030 §E).
