...
| `trace-norm` | `record{M: record{re: list<list<float64>>, im: list<list<float64>>}}` (Hermitian) | `record{value: float64, eigenvalues: list<float64>, condition_number, method, warnings}` \| `tagged "trace-norm/{non-hermitian-input,non-finite-input,degenerate-shape}"` | Schatten-1 (trace / nuclear) norm `‖M‖₁` of a complex Hermitian matrix via the spectral characterisation `‖M‖₁ = Σ \|λ_k\|` (Bhatia §IV.2). The first qinfo v0.2 tool — composes `linalg-eigh-complex` (ADR-0035 phase 1) with `Σ \|·\|`. v0.1 is Hermitian-only; the general (non-Hermitian) lane is queued for ADR-0035 phase 2 (`linalg-svd-complex`, filed under `ov4j`). Workhorse primitive for `trace-distance` (`k2xo`), `fidelity` (`2hxf`), `purity` (`2czd`); the operationally-meaningful state-distinguishability quantity (Helstrom; Fuchs–van de Graaf). See [`tools/trace-norm/`](tools/trace-norm/README.md). |
