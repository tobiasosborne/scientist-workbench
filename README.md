...
| `poly-roots` | `record{f: expression, var: symbol}` | `record{roots: list<record{root, multiplicity}>, method, warnings}` \| `tagged "poly-roots/{degree-too-high,non-polynomial,multivariate}"` | Symbolic radical roots of univariate polynomials over ℚ for degree ≤ 4. Composes `tools/poly-factor` with closed-form formulas: linear (rational root), quadratic (`(−b ± √(b² − 4ac)) / (2a)`), cubic (Cardano 1545; faithful complex form in casus irreducibilis per ADR-1yu — no trigonometric switch), quartic (Ferrari 1540 + biquadratic `q = 0` fast path). Each root is an expression Value in the closed vocabulary `+ − * / ^ neg sqrt` — exact `(−1 + √5)/2`, not `0.6180...` — composable with `cas-diff`, `integrate-1d`, and the rest of the symbolic stack. Multiplicities inherited from the factor list. Irreducible factor of degree ≥ 5 ⇒ `tagged "poly-roots/degree-too-high"` (Galois 1832; lifting cap to Root[] is bead `yoc`). Substrate `cas-core` (`linearRoot, quadraticRoots, cubicRoots, quarticRoots`). Default symbolic tier — bit-identical cross-platform forever. Validated against the 50-case `bench/poly-roots-radical/` battery (4-check verifier per ADR-0019 §1; 8 mutation perturbations; tiers A linear / B quadratic / C cubic incl. casus irreducibilis / D quartic Ferrari / E reducible / F numeric stress / G refusals; triple-witness via `bench/_corpus/oracle/` Wolfram + SymPy). |
...
| `solve` | `record{eqs: list<expression>, vars: list<symbol>}` | `record{vars, solutions: list<record{bindings, branches}>, completeness, warnings}` \| `tagged "solve/{high-degree-irreducible,multivariate-non-zero-dim,parametric-non-trivial,foreign-vocabulary,transcendental-multibranch,constant-equation,empty-input,empty-vars}"` | Top-level Mathematica `Solve[]`-class dispatcher (v0.1). Three lanes covered: linear systems → `bareissSolve` (exact Bareiss elimination over ℚ; unique / under-determined-with-branches / inconsistent verdicts); univariate polynomial → `factorRatQ` + radical solvers (deg ≤ 4 closed-form; deg ≥ 5 refused per Galois 1832); single-equation transcendental of the form `head(x) = c` for `head ∈ {exp, log, sin, cos, tan, sinh, cosh, tanh, abs}` → invert-layer with branched-solution emission (`x = arcsin(c) + 2π·t_0` ∨ `x = π − arcsin(c) + 2π·t_1` for sin, etc.); multivariate-non-zero-dim / parametric / compound-transcendental / Root[]-required → honest boundary tags pending Gröbner / substitution-heuristic / algebraic-number substrate. Output shape per ADR-0017 (solution-set: `bindings` per variable + `branches` for integer-parameter families). Branch-honest by design — no silent principal-branch slices. Substrate `@workbench/solve` (`classifyInput, dispatchClassified, tryTranscendentalInvert`) on top of cas-core / poly-factor. Default symbolic tier. Validated against the 100-case `bench/solve/` headline battery (20 hand-curated Mathematica-v1 bank + 80 stratified random across linear / univariate-poly / multivariate-zero-dim-refusal / transcendental; 4-lane dispatch verifier per ADR-0019 §1+§2; 8 mutation-prove perturbations; triple-witness via `bench/_corpus/oracle/` Wolfram + SymPy). |
...
packages/
  ...
  alg-num/               Algebraic-number substrate. Ships the `Root[poly, k]` primitive (ADR-0018): a canonical naming scheme for individual algebraic numbers via `(minimal polynomial, sort-order index)`. Two construction primitives: (1) `makeRoot(poly, intervalHint, v)` names the unique root inside a supplied real isolating interval (sign-criterion-disambiguated across irreducible factors); (2) `makeRootByIndex(poly, k, v)` names "the global k-th real root of poly in ascending order" (factors over ℚ, sorts roots cross-factor by interval bisection, re-indexes within the chosen factor). Both produce canonical Roots: irreducible minpoly, primitive, positive-leading, ℤ[x] coefficients. The wire encoding is `expression { head: "Root", args: [Polynomial[c_0, …, c_n], k] }` with all coefficients integer-typed; `valueToRoot` always defers to `makeRootByIndex` so non-canonical wire input is silently canonicalised per ADR-0018. `refineRoot(r, { bits: N })` lazily contracts the runtime isolating interval below width 2^{−N} via rational bisection — the substrate for full equality, numerical evaluation, and resultant-based arithmetic. `rootCanonicalEq` is sufficient for equality of Roots produced by any constructor (all paths canonicalise). Real roots only in v0.1 (complex-root naming awaits complex-root isolation). Resultant-based `+/·` arithmetic (rti) and primitive-element compression (5i2) extend this surface.
