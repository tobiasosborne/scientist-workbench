...
| `cas-diff` | `record{f: expression, var: symbol}` | expression \| `tagged "cas-diff/out-of-scope"` | Symbolic differentiation. Closed vocab `+ − * / ^ neg exp sin cos tan log sqrt abs asin acos atan sinh cosh tanh asinh acosh atanh log2 log10` plus `pi`, `e` — matched deliberately to `integrate-1d` / `optimize-lbfgs-projected` so the three compose without vocabulary mismatches. ADR-0023 (2026-05-08) extends recognition to the special-function vocabulary `Gamma Digamma Polygamma Bessel{J,Y,I,K} HypergeometricPFQ Whittaker{M,W} ParabolicCylinderD Erf Erfc ExpIntegral{Ei,E} Fresnel{C,S} Legendre{P,Q} LaguerreL HermiteH Chebyshev{T,U} GegenbauerC Polylog LerchPhi MeijerG`; the differentiable subset (Gamma family + Erf/Erfc + ExpIntegralEi/E + Fresnel + Bessel{J,Y,I,K} + HermiteH + Polylog) ships DLMF-cited closed-form rules in v0.1, the rest refuse honestly via the existing boundary tag pending follow-up beads. Smart constructors absorb chain-rule book-keeping (`0+x → x`, `1·x → x`, `x⁰→1`, etc.); pipe through `cas-simplify` for full reduction. Unknown head or non-arithmetic Value kind → top-level `tagged "cas-diff/out-of-scope"` (refuses the whole input rather than embedding tagged sub-nodes). FD-cross-check oracle. |
...
| `meijer-g-symbolic-only` | `record{an, ap, bm, bq: list<Value>, z: Value}` | `record{expr, rule, source, note}` \| `tagged "meijer-g-symbolic-only/no-known-reduction"` | Adamchik–Marichev + Roach symbolic dispatch for the Meijer G-function (Layer 4 of `tstournament` problem-13; ADR-0025). Pattern-table dispatcher of curated reduction rules from primary literature (Bateman §5.6 + DLMF §16.17–§16.18 in v0.1; PBM Vol 3 §8.4 + Mathai 1993 + Wolfram Functions Site land in `hv0.6.*` follow-up beads). Walks `ALL_RULES` first-match-wins; emits closed-form expressions in the cas-core special-function vocabulary (ADR-0023: `Gamma BesselJ BesselK Erf Erfc ExpIntegralEi ExpIntegralE log exp ...`); inputs canonicalised by sub-tuple-internal canonical-bytes sort so permutation within a sub-tuple matches the same rule. v0.1 ships ≥30 verified rules (every rule cross-checked against `mpmath.meijerg` at 30 dps; `dispatch-mpmath.test.ts` re-validates per-build). No-match input ⇒ `tagged "meijer-g-symbolic-only/no-known-reduction"` envelope (ADR-0003 boundary-failure). Audit grep test (`packages/meijer-core/test/dispatch-audit.test.ts`) enforces no transliteration from open-source implementation source. Substrate `@workbench/meijer-core::meijergSymbolic`. |
| `meijer-g-asymptotic-only` | `record{an, ap, bm, bq: list<bigcomplex>, z: bigcomplex}` | `record{value, achieved_precision, method, n_terms, optimal_term_indices, error_estimate, sector, working_precision, warnings}` \| `tagged "meijer-g-asymptotic-only/{stokes-line,secondary-sector,small-z,non-asymptotic-regime,no-pole-residues,input-error}"` | Braaksma far-field asymptotic for the Meijer G-function in the principal sector `\|arg z\| < π/2 − π/64` (Layer 6 of `tstournament` problem-13; ADR-0026; Braaksma 1964 *Compositio Math.* 15: 239–341 / Paris-Kaminski 2001 ch. 2 / DLMF §16.11 / Olver 1974 §3.7). v0.1 ships the **algebraic dominant asymptotic** — the n-pole Slater Series 2 truncated at its **optimal** index (Olver §3.7 "superasymptotic"): the per-pole inner-pFq series is iterated term-by-term until `\|t_{k+1}\| ≥ \|t_k\|`, and the partial sum at index `k* = k_turn` is reported with error estimate `\|t_{k*+1}\|`. Per-pole prefactor matches Slater Series 2's `B_h · z^{a_h - 1}`. Structured refusals: Stokes-line (within `2^{-workingBits/4}` of `\|arg z\| = π/2 - π/64`), secondary-sector (`\|arg z\|` past the principal cap; full Braaksma connection coefficients are `hv0.9.5`), small-z (`\|z\| < 1`; route to Slater), non-asymptotic-regime (`\|t_1\| ≥ \|t_0\|` at first step), no-pole-residues (`n = 0`; symmetric `\|z\| → 0` asymptotic is `hv0.9.4`), input-error. Determinism: `arbprec: true` — bit-identical cross-platform forever given `--precision=N`. Cross-validated against `mpmath.meijerg` at 80 dps (5 cases) and `wolframscript MeijerG` at 60 dps (5 cases); method-agreement with Slater on overlap region tested at 30 dps; 5 mutation-prove tests catch sign/prefactor/truncation/sector/recurrence regressions. Substrate `@workbench/meijer-core::meijergAsymptotic`. |
| `hypergeometric-pfq` | `record{a: list<bigcomplex>, b: list<bigcomplex>, z: bigcomplex}` + flag `--precision=<int>` | `record{value, achieved_precision, method, n_terms, working_precision, warnings}` \| `tagged "hypergeometric-pfq/{non-convergent,parameter-pole}"` | First arbprec-tier tool (ADR-0020; `arbprec: true`) — bit-identical cross-platform forever given an explicit `--precision=<int>` (default 50 dps). Evaluates the generalised hypergeometric series `pFq(a; b; z)` via direct power-series with cancellation-driven precision-bump retry; closed-form fast paths for `0F0(;;z) = exp(z)` and `1F0(a;;z) = (1-z)^{-a}`. Convergence regime: `p ≤ q` everywhere; `p == q+1` for `\|z\| < 0.99`; `p > q+1` is asymptotic-only (refused pending Borel resummation). Output is *agent-honest*: `achieved_precision` may report less than requested if cancellation exhausted the bumped budget (under-reports always honest); `working_precision` (bits) and `n_terms` are first-class diagnostic fields. `\|z\| ≥ 0.99` with `p == q+1` → `tagged "hypergeometric-pfq/non-convergent"` (boundary deferred to analytic-continuation path); `b_j` exact non-positive integer with vanishing Pochhammer → `tagged "hypergeometric-pfq/parameter-pole"`. Substrate `@workbench/hypergeometric` (refactored out in worklog 070 so `meijer-core::Slater` can share it). Validated against the 53-case `bench/hypergeometric-pfq/` battery (~282 invariant assertions across 6 tiers — closed-form anchors / generic happy / large parameters / near-unit-circle / parameter coalescence / refusals; mpmath at 80 dps + Wolfram at `precision + 30` dps cross-validated; mutation-proven). |
| `meijer-g` | `record{an, ap, bm, bq: list<bigcomplex>, z: bigcomplex, request_mode?: string}` | `record{kind: "symbolic", expr, rule, source, note, method}` \| `record{kind: "numerical", value, achieved_precision, method, working_precision, warnings, diagnostics}` \| `tagged "meijer-g/{out-of-region,non-finite-input,degenerate-shape,symbolic-required-no-match,forced-method-refused,input-error}"` | **Top-level Meijer G dispatcher** (Layer 7 of `tstournament` problem-13; ADR-0027; climax of the seven-layer stack). Composes `meijergSymbolic` + `meijergSlater` + `meijergContour` + `meijergAsymptotic` from `@workbench/meijer-core` into a single integrated evaluator with **cost-ascending dispatch** (symbolic → Slater → contour → asymptotic → refuse), **honest refusal**, and **principal-branch convention pinned** (`arg z ∈ (−π, π]`, DLMF §16.17.1). Each lane has a fast pre-filter (`canUseSlater` / `canUseContour` / `canUseAsymptotic`) that decides "applicable here?" before any numerical work runs; the dispatch loop is a flat switch over four lanes with no bespoke per-layer envelope handling. Output is a discriminated union: `kind: "symbolic"` for closed-form rule matches (Bateman §5.6 / DLMF §16.18), `kind: "numerical"` for arbprec evaluations (with `method`, `working_precision`, `warnings`, method-specific `diagnostics` including `onBranchCut` flag), or `tagged "meijer-g/<class>"` for refusals (with `ruled_out_methods` listing each refused lane). Tool-specific flags: `--force-method=<lane>` (for the method-agreement self-test), `--schwarz-check` (asserts `G(z̄) = conj(G(z))` for real-parameter non-cut z to user precision); standard `--precision=<int>` flag inherited (default 50; ADR-0020). `request_mode` field gates `auto` / `symbolic-required` / `numerical-required`. Determinism: `arbprec: true` — bit-identical cross-platform forever given `--precision=N`. The contour lane's pre-filter strengthens `canUseContour` to refuse one-sided pole clusters (`m = 0` or `n = 0`) at `\|z\| ≥ 1` because the contour-truncation cost is unbounded there; cost-ascending routes around. Wire tests (35) cover output-shape contract, method-agreement (8 cases over slater + asymptotic), pinned mpmath truths, Schwarz reflection (5 cases), branch-cut behaviour, refusal envelope, and bit-determinism. Substrate `@workbench/meijer-core::meijergDispatch`. Validated against the 91-case `bench/meijer-g/` battery (~434 invariant assertions across nine tiers — closed-form anchors / elementary symbolic / special-function numerical / generic Slater / anti-Stokes / coalescence / branch-cut / refusal / cross-cutting Tier H speed-gate; mpmath at 110 dps + Wolfram at 110 dps two-oracle consensus, Tier-0 anchors RHS-evaluated at 200 dps from the elementary closed form; mutation-proven). |
