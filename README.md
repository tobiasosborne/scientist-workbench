...
| `sdp-solve` | `record{minimize:{c}, subjectTo:{Ax_eq_b?, cones}, precision?, max_iter?}` per ADR-0030 §C with `PSDCone[size, indices]` | `record{status, x, dual, slack, objective?, achieved_precision?, iterations, method, condition_estimate, warnings}` \| `tagged "sdp-solve/{non-finite-input,degenerate-shape,quadratic-objective,non-sdp-cone,malformed-cone,cone-coverage}"` | SDP specialist of the cone-solver tier (ADR-0030 §B). Wraps `@workbench/solver-ipm`'s primal-dual interior-point method with four search-direction lanes — Nesterov-Todd default (Todd-Toh-Tütüncü 1998), AHO for A/B (Alizadeh-Haeberly-Overton 1997), HKM debug-only, and `--method=hsde-nt` (homogeneous self-dual embedding per ADR-0033 / handoff `docs/HANDOFF_solver_ipm_hsde_part2.md`, Phase 2 port; A/B-grade today, pending Phase 5 iterative refinement). Wire uses **strict-Mosek-format with √2 off-diagonal scaling** so `<C, X>_F = svec(C)ᵀ svec(X)` exactly. v0.1 cone vocabulary: PSDCone (+ ZeroCone); NonNegCone-as-diagonal-PSD deferred to v0.2 (bead `67nj`). Bench grade: 5/6 sdp-sdplib (corpus) at default tol — control2/control3 fixed by worklog 095's COPT-aligned termination + best-iterate; hinf2 holdout traced to float64 Schur conditioning, addressable by HSDE+IR per the part-2 handoff (bead `qmrv`). See [`tools/sdp-solve/`](tools/sdp-solve/README.md). |
