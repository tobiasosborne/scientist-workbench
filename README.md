...
packages/
  ...
  meijer-core/           Meijer G-function algorithmic substrate (Layer 3 of `tstournament` problem-13). Slater residue-summation (Series 1 + Series 2) with `(p, q, m, n, |z|)` selection rule, deterministic odd-coefficient perturbation for parameter coalescence, cancellation-driven retry. Composes `@workbench/bigfloat` + `@workbench/hypergeometric`. Future layers (contour quadrature, Braaksma asymptotic) land alongside.
