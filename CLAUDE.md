...
- new tool: `tools/<name>/README.md` + (if it introduces a
  value-protocol convention) a paragraph in `PRD-v0.2.md`. The catalog
  row in `docs/CATALOG.md` is *generated* — run `bun scripts/gen-catalog.ts`,
  do not hand-edit it;
- new package: `packages/<name>/README.md` + a row in the "File layout"
  block of `docs/repo-map.md`;
...
*Derived* tool-facing docs are the exception that proves the rule: per
ADR-0043 the registry is the single source of truth, so the catalog
table, the mechanical sections of each `tools/<name>/README.md`, and
goldens are *generated* (carry an `AUTOGENERATED` banner) and
drift-checked in `check.ts` — never hand-edited. The `tool.ts` literate
header and the per-tool README "How/Why" prose stay hand-written.
