...
### 3.5 Provenance write path [OPEN, NOT YET BUILT]

`runTool` does not yet emit provenance records on successful runs. The hole is small but real: the runner has the tool name, version, input hash (computable), flags, and output hash (computable). Writing the record is a one-line append in the runner. **Phase 0 ships incomplete without this.** See §10.
