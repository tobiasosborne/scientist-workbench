...
### 3.5 Provenance write path [SETTLED, BUILT]

`runTool` writes a provenance record on every successful run. The
record's shape is §3.2; the on-disk path is
`$CAS_STORE/provenance/<hh>/<output_hash>.json`; the implementation
lives in `executeToolDef` (`packages/contract/src/execute.ts`, factored
out of `runTool` per ADR-0012 so the in-process surface
(`@workbench/compose`) and the subprocess surface produce
byte-identical provenance for the same `(tool, version, input,
explicit-flags)`). Recorded `flags` carry only the explicitly-set
keys (ADR-0011); defaults are not recorded so two invocations
differing only in default-overlap produce byte-identical provenance
bytes. Provenance write failures are best-effort: a write error
surfaces as a stderr warning (subprocess) or a `console.warn`
(in-process), and never destroys the user's output.

A round-trip check is in `packages/compose/test/compose.test.ts`
("provenance record matches subprocess for the same input") —
running `mod-pow` in-process and looking the result up via
`--provenance-of` through the subprocess surface returns the same
