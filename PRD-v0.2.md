...
- **Bit-deterministic.** Same input, same version → bit-identical output. Tiered as of ADR-0015 (2026-05-04) and ADR-0020 (2026-05-07): the symbolic majority is bit-identical *cross-platform, forever* (the unconditional rule); arbitrary-precision tools annotated `arbprec: true` (ADR-0020) are *also* bit-identical *cross-platform, forever* given an explicit `--precision=<int>` flag (BigInt arithmetic is bit-identical across runtimes by language specification); numerical tools annotated `numerical: true` (ADR-0015) are bit-identical *given the platform fingerprint* (`{arch, os, runtime}`), with cross-platform divergence honestly recorded in the provenance record's `platform` field. Stochastic tools annotated `nondeterministic: true` (ADR-0005, today only `entropy-source`) opt out of the determinism contract entirely. The four annotations (default = symbolic, `arbprec: true`, `numerical: true`, `nondeterministic: true`) are mutually exclusive — enforced at execution time in `executeToolDef`; cross-tier tools declare `arbprec: true` only and wrap float64-lane results in 53-bit BigFloat (ADR-0040 §Decision 9 amendment). See `docs/adr/0015-determinism-tier.md` for the platform-fingerprint per-output tier conditioning, and `docs/adr/0020-arbitrary-precision-tier.md` for the `packages/bigfloat` substrate and the `--precision` standard-flag contract.
