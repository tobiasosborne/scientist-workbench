// =============================================================================
// scripts/mosek-log-to-jsonl.ts — convert a Mosek IPM iter log to a JSONL trace
// =============================================================================
//
...
//
//     bun scripts/mosek-log-to-jsonl.ts <mosek.log> [<out.jsonl>]
//
// If <out.jsonl> is omitted, JSONL is written to stdout. The emitted lines
// feed `scripts/trace-diff.ts`, the Mosek sibling of `copt-log-to-jsonl.ts`.
//
// NOTE: the Mosek row-format assumption is not yet verified against a real
// Mosek log — see `parseMosekLog` in `TraceLog.ts` and bead
// `scientist-workbench-yyme`.

import { readFileSync, writeFileSync } from "node:fs";
import { parseMosekLog } from "@workbench/solver-ipm";
...
function main(): void {
  ...
  const lines = parseMosekLog(readFileSync(inPath, "utf-8"));
  if (lines.length === 0) {
