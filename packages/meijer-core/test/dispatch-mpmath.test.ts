...
const CASES: XCase[] = [
  ...
  // ===========================================================================
  // BesselY canonical (bead `1xqq`, R4 §E.3 gap close).
  //
  // Canonical-Bessel slot values at ν = 3/2 (half-integer), z_g = 1:
  //   an = [], ap = [-(ν+1)/2 = -5/4], bm = [ν/2 = 3/4, -ν/2 = -3/4],
  //   bq = [-5/4]
  // mpmath: meijerg([[],[-5/4]], [[3/4, -3/4],[-5/4]], 1) = bessely(3/2, 2)
  //         ≈ -0.43459409766244371...
  // Dispatcher emits BesselY(3/2, 2·√1) = bessely(3/2, 2). Round-trip
  // expected.
  // ===========================================================================
  {
    description:
      "bessel-y-canonical: G^{2,0}_{1,3}([],[-5/4];[3/4,-3/4],[-5/4]|z=1) = Y_{3/2}(2)",
    params: {
      an: [],
      ap: [[-5, 4]],
      bm: [[3, 4], [-3, 4]],
  ...
  // ===========================================================================
  // BesselI canonical (bead `lfet`, R4 §E.3 gap close).
  //
  // Canonical-Bessel slot values at ν = 3/2 (half-integer), z_g = 1:
  //   an = [], ap = [(ν+1)/2 = 5/4], bm = [ν/2 = 3/4],
  //   bq = [-ν/2 = -3/4, (ν+1)/2 = 5/4]
  // mpmath: meijerg([[],[5/4]], [[3/4],[-3/4, 5/4]], 1)
  //         = besseli(3/2, 2) / π
  //         ≈ 1.30564... / π ≈ 0.41560...
  // Dispatcher emits (1/π)·BesselI(3/2, 2·√1) = (1/π)·besseli(3/2, 2);
  // numerically identical.
  // ===========================================================================
  {
    description:
      "bessel-i-canonical: G^{1,0}_{1,3}([],[5/4];[3/4],[-3/4,5/4]|z=1) = (1/π)·I_{3/2}(2)",
    params: {
      an: [],
      ap: [[5, 4]],
      bm: [[3, 4]],
