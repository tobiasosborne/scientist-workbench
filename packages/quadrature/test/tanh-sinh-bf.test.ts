...
//
// Cross-validation oracles for the worklog-076 hardening test bank.
...
// See worklog 076 §"Cross-validation truths" for the regeneration recipe.
const PI_110 =
...
describe("closed-form anchors (smooth integrands, Bailey §6 class 1–4)", () => {
  ...
  for (const prec of [30, 50, 100]) {
    test(`∫_0^1 t·log(1+t) dt = 1/4 at ${prec} dps`, () => {
      ...
      const a = fromInt(0n);
      const b = fromInt(1n);
      // Integrand contract (worklog 076 fix): every constant the
  ...
  for (const prec of [30, 50, 100]) {
    test(`∫_0^1 1/(1+x²) dx = π/4 at ${prec} dps  (the K15-stalls case from worklog 072)`, () => {
      ...
      const a = fromInt(0n);
      const b = fromInt(1n);
      // Integrand contract (worklog 076): `fromInt(1n, p)`, not
...
// -----------------------------------------------------------------------------
//
// Worklog 076 mandates: at 50 dps, validate against Wolfram on three
