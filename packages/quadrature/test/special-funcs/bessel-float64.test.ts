...
// -----------------------------------------------------------------------------
// §5c. Half-integer ν J/Y/K — beads z5em + omsm regression coverage
// -----------------------------------------------------------------------------
//
// Pre-fix `besselJFloat64(2.5, 50)` returned `0.02311` against mpmath
// `0.02304` (3e-3 relative error); higher half-integer ν gave worse
// (J(10.5, 100) was 5.4× wrong). Same shape for Y and K — the
// generic Hankel asymptotic / connection-formula paths weren't
// short-circuiting half-integer ν to the DLMF §10.49 closed forms.
//
// The fix (worklog 171) detects `ν = n + 1/2` via `isHalfInteger`
// and routes to the spherical-Bessel substrate: `besselJHalfInteger`,
// `besselYHalfInteger` (spherical recurrence) and
// `besselKHalfInteger` (finite-sum closed form per DLMF §10.49.13).

describe("Half-integer ν J/Y/K closed forms (beads z5em + omsm)", () => {
  // mpmath dps=25 reference values.
  type Sample = [number, number, number];

  // mpmath dps=25 reference values (all values regenerated by direct
  // `python3 -c "import mpmath; mpmath.mp.dps=25; print(...)"` to avoid
  // hand-typo errors).
  const J_REF: ReadonlyArray<Sample> = [
    [0.5, 1, 0.6713967071418031], // = sin(1)·√(2/π)
    [2.5, 1, 0.04949681022847679],
    [2.5, 50, 0.023037219509625530],
    [3.5, 50, 0.11178059493928059],
    [4.5, 50, -0.007387936218126248], // sign-sensitive
    [5.5, 50, -0.11311042345854332],
    [10.5, 50, -0.08484972094355338],
    [10.5, 100, -0.0015611238546507794],
    [20.5, 100, 0.08064754863072786], // not the small valley value
  ];
  for (const [nu, z, ref] of J_REF) {
    test(`J(${nu}, ${z}) ≤ 1e-12 (was 3e-3 to 5e+0 pre-fix)`, () => {
      const got = besselJFloat64(nu, z);
  ...
  const Y_REF: ReadonlyArray<Sample> = [
    [0.5, 1, -0.4310988680183320], // = -cos(1)·√(2/π)
    [1.5, 5, 0.32192444296114014],
    [2.5, 50, 0.11053044455625437],
    [3.5, 50, -0.0163750923062883844],
    [5.5, 50, -0.00393304003995587],
    [10.5, 50, 0.07630487814534201],
    [20.5, 100, 0.00044934699219910775],
  ];
  for (const [nu, z, ref] of Y_REF) {
    test(`Y(${nu}, ${z}) ≤ 1e-12 (was 2e-2 to 2e+0 pre-fix)`, () => {
      const got = besselYFloat64(nu, z);
      const rel = Math.abs(got - ref) / Math.abs(ref);
      expect(rel).toBeLessThan(1e-12);
    });
  }

  const K_REF: ReadonlyArray<Sample> = [
    // mpmath dps=25 values (regenerated programmatically).
    [0.5, 1, 0.4610685044478946], // = √(π/2)·e^{-1}
    [0.5, 5, 0.0037766133746428825],
    [0.5, 10, 1.799347809370518e-5],
    [1.5, 5, 0.004531936049571459],
    [1.5, 10, 1.9792825903075696e-5],
    [2.5, 5, 0.006495775004385758],
    [3.5, 5, 0.011027711053957216],
    [10.5, 5, 19.914711265368364],
  ];
  for (const [nu, z, ref] of K_REF) {
    test(`K(${nu}, ${z}) ≤ 1e-12 (was 7e-12 to 5e-7 pre-fix)`, () => {
      const got = besselKFloat64(nu, z);
      const rel = Math.abs(got - ref) / Math.abs(ref);
      expect(rel).toBeLessThan(1e-12);
    });
  }

  test("z5em point regression: K(0.5, 5) = √(π/(10))·e^{-5} matches closed form", () => {
    // mpmath dps=30: 0.0037766133746428825595...
    const got = besselKFloat64(0.5, 5);
    expect(Math.abs(got - 0.0037766133746428826) / 0.0037766133746428826).toBeLessThan(1e-15);
  });

  test("omsm point regression: J(10.5, 100) sign and order — was 5.4× wrong pre-fix", () => {
    // mpmath dps=25: -0.0015611238546507794
    const got = besselJFloat64(10.5, 100);
    expect(got).toBeLessThan(0); // sign — pre-fix returned positive
    expect(Math.abs(got)).toBeLessThan(0.01);
    expect(Math.abs(got)).toBeGreaterThan(1e-4);
    expect(Math.abs(got - -0.0015611238546507794) / 0.0015611238546507794).toBeLessThan(1e-12);
  });
});
