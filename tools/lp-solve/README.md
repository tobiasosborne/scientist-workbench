...
- **`--method=exact`** — arbitrary-precision rational engine via
  ...
  in TypeScript.
- **`--method=ipm`** — Mehrotra predictor-corrector primal-dual
  interior-point method via `@workbench/copt-ipm`, reverse-engineered
  from COPT 8.0.4. Float64 internals; scales to NETLIB (m, n ≈
  100–2000) in seconds. `numerical: true` with platform fingerprint.
...
record {
  ...
  iterations:         integer,
  method:             string,      // "simplex-q" (exact lane) | "copt-ipm" (IPM lane)
...
- [`packages/copt-ipm/`](../../packages/copt-ipm/) — the IPM engine
  (reverse-engineered TS port of COPT 8.0.4's primal-dual barrier).
- worklog 090 — the exact-lane build narrative.
- worklog 091 — the copt-ipm substrate landing.
