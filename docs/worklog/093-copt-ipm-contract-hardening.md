# 093 — `copt-ipm` workbench-contract hardening (2026-05-11)
...
(`packages/copt-ipm/test/{netlib,lp-small}.test.ts`) that called
...
### `packages/copt-ipm/src/solver/Status.ts` (new)
...
- `packages/copt-ipm/test/{netlib,lp-small}.test.ts` (new): compares
...
### `packages/copt-ipm/test/corpus.ts` (new)
...
### `packages/copt-ipm/test/netlib.test.ts` (rewritten)
...
### `packages/copt-ipm/test/lp-small.test.ts` (rewritten)
...
`@workbench/copt-ipm`. The previous duplicate definition violated
...
`@workbench/copt-ipm` covers both call sites.
...
- `bun run tsc --noEmit` clean (no copt-ipm errors).
- `bun test packages/copt-ipm/test/{netlib,lp-small}.test.ts`:
...
- `packages/copt-ipm/src/solver/Status.ts` — wire mapping.
- `packages/copt-ipm/test/corpus.ts` — portable corpus paths.
- `packages/copt-ipm/test/netlib.test.ts:24-29` — `brandy` carve-out.
- `packages/copt-ipm/test/lp-small.test.ts:24-27` — substrate gap
  carve-out.
