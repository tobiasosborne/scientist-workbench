...
| 091 | [`@workbench/copt-ipm` substrate landing (parallel-agent merge)](091-copt-ipm-substrate-landing.md) — Phase 1 of merging the COPT primal-dual IPM port (reverse-engineered from COPT 8.0.4 via Ghidra; handoff at `/home/tobias/Dropbox/.../COPT-decomp/`) onto the same-day arbprec ship.  Cherry-picks `packages/copt-ipm/` (17 src + 8 test files) from `origin/experiment/copt-ipm-port` into `merge/copt-ipm-substrate`.  Substrate-only landing: no tool integration yet, simplex-q / lp-solve / ADR-0031 / worklog-090 untouched.  Adjustments: hard-coded `/home/tobias/Projects/COPT-decomp/` test paths replaced by co-located `test/fixtures/sdp.dat-s`; `.ts` imports renamed to `.js` (workbench convention); 27 strict-null violations fixed (typed-array compound assignment under `noUncheckedIndexedAccess`).  Tests: 68/68 green.  Surfaced a pre-existing main-branch oracle failure on `lp-solve` golden 05 (filed as `2dhc`).  | 2026-05-11 | scientist-workbench-prfp (claimed, IPM lane dispatch); -{2zed, v4jd, 6or7, j1gd, 2dhc} (filed: ADR-0032, sdp-solve tool, contract hardening, algorithm hygiene, pre-existing golden bug) |
| 092 | [`tools/lp-solve` lane dispatcher (`prfp` closes)](092-lp-solve-lane-dispatcher.md) — Phase 2 of the parallel-agent merge.  Wires `@workbench/copt-ipm` into `tools/lp-solve` behind `--method=auto\|exact\|ipm` (default `auto`).  Free-variable splitting hoisted out of the engine call so both lanes see standard-form input.  Auto-dispatch threshold `m + splitN ≤ 50` → exact (bit-identical, world-first), else → IPM (NETLIB-scale).  Public wire schema unchanged; existing 12 goldens byte-identical (auto-dispatch routes all of them to exact).  Manual probe: `--method=ipm` on a 2-var LP returns `optimal` in 4 iters, `method: "copt-ipm"`.  Status taxonomy mapping: copt-ipm's `SolverStatus` collapses to ADR-0030 §A.3 wire taxonomy. | 2026-05-11 | scientist-workbench-prfp (closes) |
| 093 | [`copt-ipm` workbench-contract hardening (`6or7` closes)](093-copt-ipm-contract-hardening.md) — Phase 3 of the parallel-agent merge.  Substrate package hardened to workbench discipline: (1) NETLIB + lp-small tests rewritten from sweep-and-log to hard `expect()` assertions per case (Rule 7), `KNOWN_CONVERGENCE_GAPS = {brandy}` + `KNOWN_SUBSTRATE_GAPS = {H_malformed_cone, H_non_finite_input}` carved out with named-set comments tracking j1gd; (2) portable corpus path via `loadSuite()` (env `WORKBENCH_CORPUS` or sibling `../scientist-workbench-corpus`); (3) `--test` hook on `tools/lp-solve` exercising both lanes; (4) shared `toWireStatus` lifted to `packages/copt-ipm/src/solver/Status.ts` (single-source-of-truth between tool and tests), local `mapIpmStatus` in tool.ts deleted.  `numerical: true` + `--platform-fingerprint` already wired by the runner via `defineTool` annotation, no new code needed. 50/50 hardened tests pass with 142 `expect()` calls. | 2026-05-11 | scientist-workbench-6or7 (closes) |
