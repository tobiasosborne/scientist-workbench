...
> `@workbench/copt-ipm` substrate next to `simplex-q`; this shard
...
the additive step: the IPM engine is already in `@workbench/copt-ipm`
...
- **`solveIpmLane(args)`** builds a `CanonicalLp` from the already-
  ...
  `@workbench/copt-ipm::solveLp`, and routes the result through
  `encodeIpmResult`.
- **`encodeIpmResult`** maps the copt-ipm `SolverStatus` taxonomy
  ...
  (`achieved_precision`, `iterations`, `method: "copt-ipm"`,
  regularisation-bump warnings).
- **`METHOD_TAG`** split into `METHOD_TAG_EXACT = "simplex-q"` and
  `METHOD_TAG_IPM = "copt-ipm"` (existing references to `METHOD_TAG`
  renamed via `sed`).
...
`"simplex-q" | "copt-ipm"`. Goldens (which all hit the exact lane
...
(copt-ipm) was written assuming standard form (all `x ≥ 0`). Two
...
2. **No tool-level test file yet.** The lp-solve tool relies on
   ...
   exercise and belongs to bead `6or7` (copt-ipm contract
...
- Manual probe of `--method=ipm` on a small problem (min `x + 3y`
  ...
  `method: "copt-ipm"`, `objective ≈ 5.0`, `achieved_precision ≈
  2e-9`.
...
- `packages/copt-ipm/src/solver/Solver.ts:40-130` — the IPM main
