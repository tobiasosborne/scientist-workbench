...
ADR, worklog 076 all in lockstep. The precision-floor issue worklog 075
...
30/50/100 dps. See worklog 076 for the diagnosis snapshot and the
...
**Convergence test** (v0.1, post-worklog 076): declare convergence
...
Worklog 076 §"Frictions surfaced" documents this dead-end.
...
* Pair-generation `ε_1`: `10^{-(prec + 8)}` — the 8-dps margin below
  ...
  for integrands with `max|wf| ~ 10⁵`. Worklog 076 dead-end: with
...
* Per-level `m`-iteration: **no artificial cap** (worklog 076 fix).
